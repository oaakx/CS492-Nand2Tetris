...
class SemanticError(Exception):
  def __init__(self, msg, clname, sbname):
    self.msg = msg
    self.clname = clname
    self.sbname = sbname
...
class JackCompiler():
  ...
  def compile_subroutine(self, jclass, sb):
    ...
    if sb.kind == "constructor":
      self.out.write("push constant %s\n" % len(jclass.fields))
      self.out.write("call Memory.alloc 1\n")
      self.out.write("pop pointer 0 // alloc this segment\n")
    elif sb.kind == "method":
      sb.arguments.insert(0, ("this", jclass.name))
      self.out.write("push argument 0\n")
      self.out.write("pop pointer 0 // init this segment\n")
  ...
  def compile_let(self, jclass, sb, let):
    ...
    if vseg is None:
      raise SemanticError("Undefined variable %s" % let.vname, jclass.name, sb.name)
  ...
  def compile_variable(self, jclass, sb, var):
    ...
    if vseg is None:
      raise SemanticError("Undefined variable %s" % var.vname, jclass.name, sb.name)
  ...
  def _var_type(self, jclass, sb, var):
    # local
    scope = list(zip(*sb.locals))
    if scope and var in scope[0]:
      return scope[1][ scope[0].index(var) ]
    # argument
    scope = list(zip(*sb.arguments))
    if scope and var in scope[0]:
      return scope[1][ scope[0].index(var) ]
  ...
  def compile_call(self, jclass, sb, call):
    callself = 0
    callclass = call.container

    if call.container is None:
      self.out.write("push pointer 0\n")
      callself = 1
      callclass = jclass.name
    else:
      vtype = self._var_type(jclass, sb, call.container)
      vseg = self._var_segment(jclass, sb, call.container)
    ...
    self.out.write("call %s.%s %s\n" % (callclass, call.name, len(call.arguments) + callself))
...
def main():
  ...
  try:
    ...
    print("In class '%s', subroutine '%s'" % (e.clname, e.sbname))
    return
